...
项目采用 **手绘草稿纸** 设计风格，核心视觉特征：

- **暖纸色背景** `#FDFBF7` + 圆点网格纹理，模拟草稿纸
- **不规则圆角边框** — 所有容器使用椭圆不对称圆角（wobbly border），没有任何标准圆角
- **硬偏移阴影** — 零模糊、实心偏移，营造剪纸拼贴感
- **站酷快乐体** 标题字（ZCOOL KuaiLe），正文使用系统默认字体保证可读性
- **限定色板** — 铅笔黑 `#2D2D2D`、旧纸色 `#E5E0D8`、红色修正笔 `#FF4D4D`、蓝色圆珠笔 `#2D5DA1`、便签黄 `#FFF9C4`
- **手绘装饰** — 胶带条、图钉、虚线边框等元素

> 完整设计规范见 [DESIGN_SYSTEM.md](DESIGN_SYSTEM.md)，新增页面时请严格遵循。
...
│  └─ add_plot_sheet.dart
├─ theme/
│  └─ app_theme.dart               # 手绘风格设计 Token 系统
└─ utils/
...
所有 UI 组件遵循统一的手绘草稿纸设计系统。详细规范（含代码模板）见 [DESIGN_SYSTEM.md](DESIGN_SYSTEM.md)。

核心文件：
- `lib/theme/app_theme.dart` — 色彩、圆角、阴影、字体等设计 Token
- `lib/main.dart` — 纸张纹理背景 (`_PaperTexturePainter`)
- `DESIGN_SYSTEM.md` — 完整设计规范文档，新增页面时参考
...
- 新增页面时严格遵循 [DESIGN_SYSTEM.md](DESIGN_SYSTEM.md) 中的设计规范
